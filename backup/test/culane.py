...
def testing(db, nnet, result_dir, debug=False, evaluator=None, repeat=1,
    ...
    print("開始進行純 Python 端 CULane 指標評估...")

    # 假設你的 DataLoader 迭代所有測試資料
    for b_idx, data in enumerate(db_loader):
        images = data['images'].cuda()
        # 模型推理
        outputs = nnet(images) 
        
        # 解析預測出的車道線坐標 (依據你原本的解碼邏輯)
        pred_lanes = json_to_lanes(outputs) 
        # 取得對應的 Ground Truth 車道線坐標
        gt_lanes = data['gt_lanes'] 
        
        # 呼叫純 Python 評估核心
        res = evaluate_culane_pure_python(pred_lanes, gt_lanes, img_shape=(590, 1640))
        total_tp += res['tp']
        total_fp += res['fp']
        total_fn += res['fn']

    # 迴圈結束後，直接計算並列印 SoTA 指標
    precision = total_tp / (total_tp + total_fp + 1e-6)
    recall = total_tp / (total_tp + total_fn + 1e-6)
    f1_score = 2 * (precision * recall) / (precision + recall + 1e-6)

    print("\n========= CULane 純 Python 評估結果 =========")
